...
const Counter = () => {
    ...
    const fetchCpuData = () => {
      fetch("/api/server", { credentials: "include" })
        ...
        .then((data) => {
          ...
          setCpuOption((prevOption) => {
            const updatedOption = JSON.parse(JSON.stringify(prevOption));

            if (!updatedOption.xAxis[0].data) updatedOption.xAxis[0].data = [];
            if (!updatedOption.series[0]) {
              updatedOption.series.push({
                name: "People Counter (Right)",
                type: "line",
                yAxisIndex: 0,
                data: [],
              });
            }
            if (!updatedOption.series[1]) {
              updatedOption.series.push({
                name: "People Counter (Left)",
                type: "line",
                yAxisIndex: 0,
                data: [],
              });
            ...
            if (updatedOption.xAxis[0].data.length > 30) {
              updatedOption.xAxis[0].data.shift();
              updatedOption.series[0].data.shift();
              updatedOption.series[1].data.shift();
            }

            updatedOption.xAxis[0].data.push(time);
            updatedOption.series[0].data.push(data.peopleCounter.right || 0); // Right counter
            updatedOption.series[1].data.push(data.peopleCounter.left || 0);  // Left counter
  ...
  const videoUrl = "/live/counter.flv"; // Placeholder for the stream URL
  const [password, setPassword] = i.a.useState("");
  const cookies = new H.a();

  i.a.useEffect(() => {
    // Set the password from cookies or default to an empty string
    setPassword(cookies.get("pass") || "");
  }, []);

  // Function to generate a signed URL for secure video streaming
  const getSignedUrl = () => {
    if (!password) return videoUrl; // Return the base URL if no password is set
    const hash = F.a.create(); // Create a hash object
    const expiry = Date.now() + 30000; // Set the expiry time to 30 seconds from now
    hash.update(`/live/counter-${expiry}-${password}`); // Update the hash with the URL path and password
    const signature = hash.hex(); // Generate the hash signature
    return `${videoUrl}?sign=${expiry}-${signature}`; // Return the signed URL
  };

  // Render the video player and chart components
  return i.a.createElement(
    "div",
    { className: "ant-row", style: { margin: "0px -12px" } },
    i.a.createElement(
      "div",
      { style: { padding: "0px 12px" }, className: "ant-col ant-col-12" },
      i.a.createElement(z, {
        url: getSignedUrl(), // Pass the signed URL to the video player
        type: "flv", // Specify the video type
        style: { width: "100%", maxWidth: "800px", height: "auto", marginBottom: "16px" }, // Set the video player style
      })
    ),
    i.a.createElement(
      "div",
      ...
      i.a.createElement(
        ...
        i.a.createElement(
          ...
          i.a.createElement(
            "div",
            {
              style: { height: "300px", width: "100%", userSelect: "none", position: "relative" },
              className: "echarts-for-react", // Chart container class
            },
            i.a.createElement(M.a, {
              echarts: S.a, // Pass the echarts instance
              option: cpuOption, // Pass the chart options
              notMerge: true, // Do not merge with previous options
              lazyUpdate: false, // Disable lazy updates
              style: { height: "300px", width: "100%" }, // Set the chart style
            })
          )
        )
      )
